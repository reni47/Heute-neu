...
/* ============================================ */
/* MASSIVE HERZ-REGEN BEIM WEBSITE-START */
/* ============================================ */

function startMassiveHeartRain() {
    const container = document.createElement('div');
    container.className = 'massive-heart-rain';
    document.body.appendChild(container);
    
    // 200 Herzen auf einmal!
    for (let i = 0; i < 200; i++) {
        setTimeout(() => {
            const heart = document.createElement('div');
...
/* ============================================ */
/* "DRÜCK MICH" BUTTON - FETTER HERZ-REGEN */
/* ============================================ */

function initDruckMichButton() {
    const druckMichBtn = document.getElementById('mainHeartBtn');
    const themePickerBtn = document.getElementById('themePickerBtn');
    
    if (druckMichBtn) {
        druckMichBtn.addEventListener('click', (e) => {
            // FETTER Herz-Regen
            startMassiveHeartRain();
            
            // Extra Herzen am Button
            const rect = e.target.getBoundingClientRect();
            popHearts(rect.left + rect.width / 2, rect.top + rect.height / 2, 50);
        });
    }
    ...
    if (themePickerBtn) {
        themePickerBtn.addEventListener('click', (e) => {
            switchTheme();
            const rect = e.target.getBoundingClientRect();
            popHearts(rect.left + rect.width / 2, rect.top + rect.height / 2, 20);
        });
    }
}

// Bei Main Content Init aufrufen
const originalInitMainContent = initMainContent;
window.initMainContent = function() {
    originalInitMainContent();
    initDruckMichButton();
    
    // Massiver Herz-Regen beim Seitenstart (nach Celebration)
    setTimeout(() => {
        startMassiveHeartRain();
    }, 500);
};
